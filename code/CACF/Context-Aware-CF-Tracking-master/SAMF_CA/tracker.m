...
% 	window_sz = 2 .^ nextpow2(window_sz);
	...
	%增加了搜索窗口大小的动态调整（通过search_size数组），适应目标大小的变化。
...
% 		end
		...
		if frame > 1,
            ...
            for i=1:size(search_size,2) %数组第二维度的大小（即列数）。
                ...
                param0 = affparam2mat(param0); %将参数转换为仿射变换矩阵
                patch = uint8(warpimg(double(im), param0, window_sz)); %提取变换后的图像区域，即新的搜索窗口。
                zf = fft2(get_features(patch, features, cell_size, cos_window,w2c));
                %沿着第三维（通常对应于不同的特征或通道）求和，生成一个综合的响应；real 函数用于提取这些复数中的实部
