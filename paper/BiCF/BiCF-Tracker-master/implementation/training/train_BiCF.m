...
% xf:特征图, yf:标签图, s:空间正则，wf_p:滤波器
% 计算中间变量 Sxy、Sxx、x_fuse 和 Sxx_fuse
% output = cellfun(function_handle, cell_array, 'UniformOutput', uniform_output_flag)
Sxy = cellfun(@(xf, yf) bsxfun(@times, xf, conj(yf)), xf, yf, 'UniformOutput', false); % 结果仍是元胞数组
